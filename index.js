function validateIE(inscricaoEstadual, UF) {
    ...
    switch (UF) {
        ...
        // Rio Grande do Norte
        case 'RN':
            if (tamanho != 9 && tamanho != 10) return false;
            pesos = tamanho == 10 ? [10, 9, 8, 7, 6, 5, 4, 3, 2] : [9, 8, 7, 6, 5, 4, 3, 2];
            soma = 0;
            ...
            if (digitoVerificador != inscricaoEstadual[tamanho - 1]) return false;
        ...
        case 'RS':
            ...
            if (digitoVerificador != inscricaoEstadual[9]) return false;
        ...
        case 'RO':
            ...
            if (digitoVerificador == 10 || digitoVerificador == 11) digitoVerificador -= 10;

            if (digitoVerificador != inscricaoEstadual[13]) return false;
...
const formatIE = (ie, uf) => {
    switch (uf) {
        ...
        case 'RN':
            return ie.length == 9
                ? `${ie.substring(0, 2)}.${ie.substring(2, 5)}.${ie.substring(5, 8)}-${ie.substring(8, 9)}`
                : `${ie.substring(0, 2)}.${ie.substring(2, 3)}.${ie.substring(3, 6)}.${ie.substring(6, 9)}-${ie.substring(9, 10)}`

        // Rio Grande do Sul
        case 'RS':
            return `${ie.substring(0, 3)}/${ie.substring(3, 10)}`;
        
        // Rondônia
        case 'RO':
            return `${ie.substring(0, 12)}-${ie.substring(12, 13)}`;
...
console.log(validateAndFormatIE('79527507949646', 'RO'));